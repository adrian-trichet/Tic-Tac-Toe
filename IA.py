...
# Fonction pour le meilleur coup de l'IA
def best_move(board):
    best_score = -float('inf')
    move = None
    for row in range(3):
...
def delayed_ai_move(board, buttons):
    root.after(2000, ai_move, board, buttons)  # 2000 ms = 2 secondes
...
def player_move(row, col, board, buttons):
    if board[row][col] == ' ':
        ...
        if check_victory(board):
            ...
            delayed_ai_move(board, buttons)  # Délai avant que l'IA joue
    ...
    for row in range(3):
        for col in range(3):
            buttons[row][col] = tk.Button(root, text=' ', font=('normal', 40), width=5, height=2,
                                          command=lambda r=row, c=col: player_move(r, c, board, buttons))
            buttons[row][col].grid(row=row, column=col)

    reset_button = tk.Button(root, text="Réinitialiser", font=('normal', 20), command=lambda: reset_game(board, buttons))
